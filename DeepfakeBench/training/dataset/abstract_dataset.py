# Adapted from the DeepfakeBench dataset implementation.
# Modified for Effort++.
...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    def __init__(self, config=None, mode='train'):
        ...
        Args:
            ...
            mode (str): A string indicating the mode (train, val, or test).
        ...
        self.frame_num = config['frame_num'].get(mode, config['frame_num']['test'])
        ...
        if mode == 'train':
            ...
            dataset_key = 'validation_dataset' if mode == 'val' else 'test_dataset'
            one_data = config[dataset_key]
            # Validation and test datasets are evaluated separately.
    ...
    def init_data_aug_method(self):
        ...
        Default (data_aug_from_config=false): legacy hardcoded Compose.
        When data_aug_from_config=true: build from config['data_aug'] keys
        (flip/rotate/blur/brightness/contrast/quality).
        """
        if self.config.get('data_aug_from_config', False):
            return self._init_data_aug_from_config()
        return self._init_data_aug_legacy()

    def _init_data_aug_legacy(self):
        return A.Compose([
    ...
    def _init_data_aug_from_config(self):
        cfg = dict(self.config.get('data_aug') or {})

        def _limit(val, default):
            if val is None:
                return default
        ...
        flip_p = float(cfg.get('flip_prob', 0.5))
        rotate_p = float(cfg.get('rotate_prob', 0.0))
        rotate_limit = _limit(cfg.get('rotate_limit'), 10)
        if isinstance(rotate_limit, tuple):
            rotate_arg = rotate_limit
        else:
            rotate_arg = int(rotate_limit)

        blur_p = float(cfg.get('blur_prob', 0.0))
        blur_limit = cfg.get('blur_limit', 3)
        if isinstance(blur_limit, (list, tuple)) and len(blur_limit) == 2:
            blur_limit = (int(blur_limit[0]), int(blur_limit[1]))
            motion_blur_limit = blur_limit[1]
        else:
            blur_limit = int(blur_limit)
            motion_blur_limit = blur_limit

        bright_p = float(cfg.get('brightness_prob', 0.5))
        bright_limit = _limit(cfg.get('brightness_limit'), 0.1)
        contrast_limit = _limit(cfg.get('contrast_limit'), 0.1)

        q_low = int(cfg.get('quality_lower', 40))
        q_high = int(cfg.get('quality_upper', 100))
        compress_p = float(cfg.get('compression_prob', 0.5))

        transforms = [A.HorizontalFlip(p=flip_p)]
        if rotate_p > 0:
            transforms.append(A.Rotate(limit=rotate_arg, p=rotate_p))
        transforms.append(
    ...
    def load_rgb(self, file_path):
        ...
        if not self.lmdb:
            # 修复路径处理逻辑
            if 'dataset_root_rgb' in self.config:
                # 将Windows路径格式转换为Linux格式
                file_path = file_path.replace('\\', '/')
                # 组合完整路径
                full_path = os.path.join(self.config['dataset_root_rgb'], file_path)
            else:
                full_path = file_path
            ...
            if img is None:
                ...
                if img is None:
                    raise ValueError('Loaded image is None: {}'.format(full_path))
        elif self.lmdb:
