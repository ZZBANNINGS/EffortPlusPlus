...
def extract_aligned_face_dlib(face_detector, predictor, image, res=224, mask=None):
    ...
    if len(faces):
        ...
        landmark = np.array([(point.x, point.y) for point in landmark.parts()])
...
def infer_single_image(
    ...
    cls_out = int(preds["cls"].argmax(dim=1).item())
    prob = float(preds["prob"].item())
    return cls_out, prob
...
def parse_args():
    ...
    p.add_argument("--detector_config", default='training/config/detector/effort_svd_lu_slerp_cos.yaml',
                   help="YAML 配置文件路径")
...
def main():
    ...
    for idx, img_path in enumerate(img_paths, 1):
        ...
        try:
            ...
            print(f"[Warning] {img_path}: {exc}", file=sys.stderr)
            continue
        print(
